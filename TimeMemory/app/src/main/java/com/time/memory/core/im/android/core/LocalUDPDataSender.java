 ...
 */
package com.time.memory.core.im.android.core;
...
import com.time.memory.core.im.android.utils.UDPUtils;
import com.time.memory.core.im.protocal.CharsetHelper;
import com.time.memory.core.im.protocal.ErrorCode;
import com.time.memory.core.im.protocal.Protocal;
import com.time.memory.core.im.protocal.ProtocalFactory;
...
public class LocalUDPDataSender {
	...
	public int sendLogin(String loginName, String loginPsw, String extra) {
		byte[] b = ProtocalFactory.createPLoginInfo(loginName, loginPsw, extra).toBytes();
		int code = send(b, b.length);
		CLog.e(TAG, "LocalUDPDataSender:" + code);
		// 登陆信息成功发出时就把登陆名存下来
		if (code == 0) {
...
//				// 重置登陆标识
		...
		IMClientManager.getInstance(MainApplication.getContext()).setInit(false);
		return code;
	}
	...
	public int sendCommonData(byte[] dataContent, int dataLen, String to_user_id) {
		return sendCommonData(
				CharsetHelper.getString(dataContent, dataLen), to_user_id, false, null);
	}

	public int sendCommonData(byte[] dataContent, int dataLen, String to_user_id, boolean QoS, String fingerPrint) {
		return sendCommonData(
	...
	public int sendCommonData(String dataContentWidthStr
			, String to_user_id, boolean QoS, String fingerPrint) {
		return sendCommonData(ProtocalFactory.createCommonData(dataContentWidthStr,
...
//				ds.connect(new InetSocketAddress(ConfigEntity.serverIP, ConfigEntity.serverUDPPort));
				CLog.e(TAG, "ds.isConnected():" + ds.isConnected());
			} catch (Exception e) {

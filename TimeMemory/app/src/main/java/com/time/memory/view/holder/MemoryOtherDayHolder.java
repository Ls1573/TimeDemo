 ...
 */
public class MemoryOtherDayHolder extends BaseHolder<OtherMemory> {
	...
	public MemoryOtherDayHolder(View view) {
		super(view);
	}
	 ...
	 */
	@Override
	public void setData(final OtherMemory entity, int positoin) {
		this.setData(mData);
		...
		//人物图片
		if (!TextUtils.isEmpty(entity.getHeadPhotoF()))
			Picasso.with(mContext).load(imgPath + entity.getHeadPhotoF() + imageOss).transform(new CircleImageTransformation()).error(R.drawable.login_photo).placeholder(R.drawable.login_photo).into(time_marker);
		else
		...
		//来源
		time_from_tv.setText(entity.getUserNameF());
		//题头

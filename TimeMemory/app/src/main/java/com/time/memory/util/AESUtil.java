 ...
 * @author Qiu
 * 
 * @Package com.pokeepera.bedook.utils
 * 
 * @Description:AES对称加密算法
 * 
 * @date 2016-7-19 下午4:31:34
 * 
 * @version 1.0 ==============================
 */
public class AESUtil {

	static final String algorithmStr = "AESUtil/ECB/PKCS5Padding";

	private static final Object TAG = "AESUtil";
	...
	private static void init() {
		try {
			 ...
			 */
			keyGen = KeyGenerator.getInstance("AESUtil");
	...
	private static byte[] encrypt(byte[] content, byte[] keyBytes) {
		 ...
		 */
		Key key = new SecretKeySpec(keyBytes, "AESUtil");
		try {
			// 用密钥初始化此 cipher。
	...
	private static byte[] encrypt(String content, String password) {
		try {
			...
			SecretKeySpec key = new SecretKeySpec(keyStr, "AESUtil");

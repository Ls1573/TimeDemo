...
public class NineGridImageView<T> extends ViewGroup {
	...
	public NineGridImageView(Context context, AttributeSet attrs) {
		...
		this.isBorder = typedArray.getBoolean(R.styleable.NineGridImageView_hasborder, false);
		typedArray.recycle();
	}
...
//                mImageViewList.get(0).setScaleType(ImageView.ScaleType.CENTER_CROP);
	...
	protected void onDetachedFromWindow() {
		...
		if (mImageViewList != null) {
			int siez = mImageViewList.size();
			for (ImageView imgView : mImageViewList) {
	...
	private void layoutChildrenView() {
		...
		for (int i = 0; i < childrenCount; i++) {
			...
			if (mAdapter != null) {
				mAdapter.onDisplayImage(getContext().getApplicationContext(), childrenView, mImgDataList.get(i));
			}
	...
	private ImageView getImageView(final int position) {
		if (position < mImageViewList.size()) {
			...
			if (mAdapter != null) {
				ImageView imageView = mAdapter.generateImageView(getContext().getApplicationContext(), isBorder);

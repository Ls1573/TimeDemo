...
import com.time.memory.view.holder.GroupTimeHolder;
import com.time.memory.view.holder.GroupTimeItemHolder;
 ...
 * @author @Qiu
 * @version V1.0
 * @Description:群的时间轴(第二次修改)
 * @date 2016/11/9 16:25
 */
public class GroupTimeAdapter extends RecyclerView.Adapter<BaseHolder> implements SectionAdapter<GroupTimeAdapter.Section>, IHolderCallBack {

	private static final String TAG = "TimeAdapter";

	private static final int VIEW_TYPE_FOOTER = 0x02;
	...
	public void addAll(List<GroupMemorys> list) {
		convertData(list);
...
//			notifyItemChanged(getItemCount() - 1);
	...
	//转换数据
	private void convertData(List<GroupMemorys> mlist) {
		if (getItemCount() != 0) {
			mSectionGraph.getItem(getItemCount() - 1).setIsLast(false);
		}
		Section currentSection;
		for (int i = 0; i < mlist.size(); i++) {
			GroupMemorys OtherMemorys = mlist.get(i);
			...
			//内容
			for (GroupMemory memory : mlist.get(i).getList()) {
				currentSection.addItem(new GroupMemorys(memory, false, mlist.get(i).getStart(), memory.getPosition()));
			}
		}
	...
	public BaseHolder onCreateViewHolder(ViewGroup parent, int viewType) {
		...
		if (viewType == VIEW_TYPE_FOOTER) {
			view = LayoutInflater.from(parent.getContext())
					.inflate(R.layout.recycler_load_more_layout, parent, false);
			return new CommonFooterHolder(view);
		} else if (viewType == VIEW_TYPE_HEADER) {
			view = LayoutInflater.from(parent.getContext())
					.inflate(R.layout.item_time_header, parent, false);
			return new GroupTimeHolder(view);
		} else {
			view = LayoutInflater.from(parent.getContext())
					.inflate(R.layout.item_memory_sign, parent, false);
			return new GroupTimeItemHolder(view);
		}
	}
	...
	public void onBindViewHolder(BaseHolder holder, int position) {
		...
		GroupMemorys item = mSectionGraph.getItem(position);
	...
	private void notifyHeaderChanges() {
		...
		for (int i = 0; i < count; i++) {
			final GroupMemorys item = mSectionGraph.getItem(i);
			if (item.isHeader) {
				notifyItemChanged(i);
			}
		}
	...
	public static class Section extends SectionAdapter.Section<Section> {
		...
		private ArrayList<GroupMemorys> mItems = new ArrayList<>();

		private GroupMemorys mHeader;
		...
		public Section addHeader(GroupMemorys header) {
			mHeader = header;
			return this;
		}

		public Section addItem(GroupMemorys item) {
			mItems.add(item);
			return this;
		}
		...
		public GroupMemorys getItem(int position) {

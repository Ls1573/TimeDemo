...
import com.time.memory.gui.gallery.utils.MediaScanner;
import com.time.memory.gui.gallery.utils.Utils;
import com.time.memory.view.activity.memory.SupportActivity;
import com.time.memory.view.activity.memory.WriterMActivity;
...
public abstract class PhotoBaseActivity extends FragmentActivity implements EasyPermissions.PermissionCallbacks {
	...
	protected int initColor() {
		return getResources().getColor(R.color.grey_F8);
	}
	 ...
	 * 拍照获取权限
	 */
	@AfterPermissionGranted(GalleryFinal.TAKE_REQUEST_CODE)
	protected void requestTakePermission() {
		if (EasyPermissions.hasPermissions(this, Manifest.permission.CAMERA)) {
			...
			EasyPermissions.requestPermissions(this, getString(R.string.permissions_tipes_takephoto),
					GalleryFinal.TAKE_REQUEST_CODE, Manifest.permission.CAMERA, Manifest.permission.WRITE_EXTERNAL_STORAGE);
		}
	}
		...
		if (requestCode == GalleryFinal.TAKE_REQUEST_CODE) {
			if (resultCode == RESULT_OK && mTakePhotoUri != null) {
				...
				if (new File(path).exists()) {
					...
					info.setPhotoD(Utils.getRandom(10000, 99999));
					info.setPhotoPath(mTakePhotoUri.toString());
					updateGallery(path);
					takeResult(info);
				} else {
					takePhotoFailure();
			...
			Toast.makeText(this, "从设置页回到App", Toast.LENGTH_SHORT)
					.show();
		}
	}
	 ...
	 * @param photoList
	 */
	protected void resultWriter(ArrayList<PhotoInfo> photoList, int state, String Id) {
		Intent intent = new Intent(getApplicationContext(), WriterMActivity.class);
		intent.putParcelableArrayListExtra("photos", photoList);
		intent.putExtra("state", state);
		intent.putExtra("class", GalleryFinal.getFunctionConfig().getOprClass());
	 ...
	 * @param photoList
	 */
	protected void resultSuppory(ArrayList<PhotoInfo> photoList, int state, String Id) {
		Intent intent = new Intent(getApplicationContext(), SupportActivity.class);
		intent.putParcelableArrayListExtra("photos", photoList);
		intent.putExtra("state", state);
		intent.putExtra("Id", Id);
		intent.putExtra("memoryId", GalleryFinal.getFunctionConfig().getMemoryId());
		intent.putExtra("memorySourceId", GalleryFinal.getFunctionConfig().getMemorySourceId());
		intent.putExtra("userId", GalleryFinal.getFunctionConfig().getUserId());
	...
	public void onPermissionsDenied(int requestCode, List<String> perms) {
		...
		if (EasyPermissions.somePermissionPermanentlyDenied(this, perms)) {
			new AppSettingsDialog.Builder(this, getString(R.string.rationale_ask_again))
					...
					.setNegativeButton(getString(R.string.app_cancle), null /* click listener */)

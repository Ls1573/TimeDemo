...
import com.time.memory.view.fragment.memory.own.MemoryHotFragment;
import com.time.memory.view.fragment.memory.own.MemoryTagFragment;
import com.time.memory.view.fragment.memory.own.MemoryTimeFragment;
 ...
 * @version V1.0
 * @Description:我的记忆
 * @date 2016-9-9下午12:02:40
 * ==============================
 */
public class MyMemoryActivtiy extends BaseActivity implements IBaseView {

	private static final String TAG = "MyMemoryActivtiy";
	...
	private MemoryHotFragment allMemoryFragment;//(热度)
	private MemoryTimeFragment monthMemoryFragment;//(时间轴)
	private MemoryTagFragment tagFragment;// 标签
...
//		setDragEdge(SwipeBackLayout.DragEdge.LEFT);
	...
	public void initData() {
		...
		mFragmentList.add(allMemoryFragment = new MemoryHotFragment());
		mFragmentList.add(monthMemoryFragment = new MemoryTimeFragment());
		mFragmentList.add(tagFragment = new MemoryTagFragment());
	...
	private void initIndicator() {
		...
		commonNavigator.setAdapter(new CommonNavigatorAdapter() {
			...
			public IPagerIndicator getIndicator(Context context) {
				...
				linePagerIndicator.setLineHeight(DevUtils.dip2px(mContext.getApplicationContext(), 2));
	...
	public void showPicDialog() {
		// dialog弹窗
		ActionSheet.createBuilder(mContext.getApplicationContext(), getSupportFragmentManager())
				...
				.setListener(new ActionSheet.ActionSheetListener() {
					...
					public void onOtherButtonClick(ActionSheet actionSheet, int index) {
						switch (index) {
							...
							case 1:
								//相册选取
								GalleryFinal.openGalleryMuti(REQUEST_CODE_GALLERY, initPhoto(999, false, true, false, type, mGroup.getGroupId(), mPhotoList), null);
								break;
						}
					}
